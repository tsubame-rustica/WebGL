...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    userId = urlParams.get('id');  // ?id= の値を取得

    pollInterval = setInterval(() => pollServerWithId(), 1000);
});

// ポーリングでサーバーからデータを取得する関数
function pollServerWithId() {
    ...
    .then(data => {
        console.log(`サーバーからのレスポンス (ID: ${userId}) :`, data);
        if (data.state === 'inProgress') {  // admin.jsでスタートが押された
            startTime = data.startTime;
            clearInterval(pollInterval);
            startAppInteral = setInterval(checkStartAppTime(), 10);
            animate();
        } else if (data.state === 'waiting') {  // 待機する
            console.log("waiting...")
        }
    })
    .catch(error => {
...
// 開始時間になるまでインターバルで実行
function checkStartAppTime() {
    // 指定された時間（フォーマット 'HH:MM:SS'）
    const [hours, minutes, seconds] = startTime.split(':').map(Number);
    
    // 現在時刻を取得
    const currentDate = new Date();
    
    // 指定された時間のDateオブジェクトを作成
    const specifiedDate = new Date();
    specifiedDate.setHours(hours, minutes, seconds, 0);  // ミリ秒は0にセット
    
    console.log(currentDate, specifiedDate);
    // 開始時間になったら実行
    if (currentDate >= specifiedDate) {
        const cardOuter = document.getElementById("cardOuter");
        cardOuter.style.display = "none";
        clearInterval(startAppInteral); // インターバルを終了
        return;
    }
} 
